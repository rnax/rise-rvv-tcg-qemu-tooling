/* Wrapper for benchmarking memmove
...
#include <string.h>
#else
extern int memmove_v (const void *dest, const void *src, size_t n);
#endif
...
benchmark_wrapper (size_t size, size_t iters)
{
  ...
  for (size_t i = 0; i < iters; i++)
    {
#ifdef STANDARD_LIB
      memmove((const void *) dst, (const void *) src, size);
#else
      memmove_v((const void *) dst, (const void *) src, size);
#endif
    }
}
