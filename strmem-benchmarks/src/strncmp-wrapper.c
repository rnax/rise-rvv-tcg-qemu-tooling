/* Wrapper for benchmarking strncmp
...
#include <string.h>
#else
extern int strncmp_v (const char *s1, const char *s2, size_t n);
#endif
...
/* Benchmark strncmp
   ...
   scanned.  We always allocate blocks of twice the size.
...
benchmark_wrapper (size_t size, size_t iters)
{
  ...
  for (size_t i = 0; i < DATASETS; i++)
    {
      data1[i] = malloc (size + size + 1);
      data2[i] = malloc (size + size + 1);
      str_init_random (data1[i], size + size);
      str_init_random (data2[i], size + size);
    }
...
#ifdef STANDARD_LIB
      res[ds] = strncmp (data1[ds], data2[ds], size);
#else
      res[ds] = strncmp_v (data1[ds], data2[ds], size);
#endif
    }
}
